...
function listDatas(size) {
	...
	$.extend($.common.plugin.jqGrid.settings({size: size}), {
		url: ctx + '/js/module/policy/policy-car-import.json',
		colNames: [' ', '保单编号', '车主姓名', '性别', '年龄', '车牌号', '注册登记日期', '行驶城市', '购车时间', '车价(万)', '生效日期', '结束日期', '联系人', '手机', 'Email'],
        colModel: [{
			name: 'options',
			width: 60,
			fixed: true,
...
function showSchemeCarList(size) {
	...
	$.extend($.common.plugin.jqGrid.settings({
		...
		colNames: ['车主姓名', '性别', '年龄', '车牌号', '注册登记日期', '行驶城市', '购车时间', '车价(万)', '联系人', '手机', 'Email', '相关文件'],
        colModel: [{
			name: 'name',
			align: 'center',
			width: 60,
