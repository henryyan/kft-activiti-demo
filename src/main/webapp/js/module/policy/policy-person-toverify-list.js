...
function listDatas(size) {
	...
	$.extend($.common.plugin.jqGrid.settings({size: size}), {
		url: ctx + '/js/module/policy/policy-person-toverify.json',
		colNames: ['保单编号', '投保人姓名', '性别', '出生日期', '身份证号', '健康状况', '投保日期', '现住地','手机号码','Email', '生效日期', '结束日期', '审核'],
        colModel: [{
			name: 'policyNumber',
			align: 'center',
			width: 80
		...
		subGridRowExpanded: function(subgrid_id, row_id){
			...
			$("#"+subgrid_table_id).jqGrid({
				url: ctx + "/js/module/scheme/scheme-person-toverify-data.json",
				datatype: "json", 
				...
				colModel: [{
					...
					edittype: 'select',
					editoptions: {
						value: {"新华美满人生保险": "新华美满人生保险", "平安附加重疾终身保险": "平安附加重疾终身保险", "平安全福保本终身保险": "平安全福保本终身保险"}
					}
				}, {
					name: 'total',
					editable: true,
