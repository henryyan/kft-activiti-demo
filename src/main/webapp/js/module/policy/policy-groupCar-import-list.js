...
function listDatas(size) {
	...
	$.extend($.common.plugin.jqGrid.settings({size: size}), {
		url: ctx + '/js/module/policy/policy-groupCar-import.json',
		colNames: [' ', '保单编号', '企业名称', '法人代表', '企业性质', '成立时间', '员工人数', '注册登记日期', '车辆信息',  '生效日期', '结束日期', '联系方式', 'Email'],
        colModel: [{
			name: 'options',
			width: 60,
			fixed: true,
...
function showSchemeCarList(size) {
	...
	$.extend($.common.plugin.jqGrid.settings({
		...
		url: ctx + '/js/module/scheme/scheme-groupCar-data.json',
		colNames: ['企业名称', '法人代表', '企业性质', '成立时间', '员工人数', '注册登记日期', '车辆信息', '联系方式', 'Email', '相关文件'],
        colModel: [{
			name: 'cpaName',
			align: 'center',
			width: 60,
