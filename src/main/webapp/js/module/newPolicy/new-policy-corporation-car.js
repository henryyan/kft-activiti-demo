...
var moduleAction = 'newPolicy/new';
...
function listDatas(size) {
	...
	$.extend($.common.plugin.jqGrid.settings({size: size}), {
		url: ctx + '/js/module/newPolicy/new-policy-corporation-car.json',
		colNames: ['保单编号', '企业名称', '法人代表', '企业性质', '成立时间', '员工人数', '注册登记日期', '车辆信息', '联系电话', 'Email', '审核', '编制文档'],
        colModel: [{
			name: 'policyNumber',
			align: 'center',
			width: 80,
			...
			formatter: function(cellvalue, options, rowObject) {
        		return "<button class='markPotential'>查看</button> ";
        	}
		},{
			name: 'mobilePhone',
			width: 90,
...
function selectCompany(){
	if($("#insuranceCompany").val() == "天平汽车保险有限公司"){
		$("#riskType").html("<option>全车盗抢险 </option><option>单独玻璃险</option>");
	}else if($("#insuranceCompany").val() == "上海安国汽车保险有限公司"){
		$("#riskType").html("<option>划痕险  </option><option>新设备增加险</option>");
	}else if($("#insuranceCompany").val() == "太平洋汽车保险有限公司"){
		$("#riskType").html("<option>自燃损失险</option><option>新设备增加险</option>");
	}else if($("#insuranceCompany").val() == "上海安普信汽车保险有限公司"){
		$("#riskType").html("<option>自燃损失险</option><option>新设备增加险</option>");
	}else{
...
function subInquirAndQuoteGrid(subgrid_id, row_id) {
	...
	$("#"+subgrid_table_id).jqGrid({
		...
		colModel: [{
			...
			formatter: function(cellvalue, options, rowObject) {
				if (cellvalue == 'false') {
					return "<img src='" + ctx + "/images/tip/err.gif' class='err-qtip' title='人身险优惠降低2个点'/>";
				} else if (cellvalue == "") {
					// 业务人员
					if (role == 'businessMan') {
...
function insuranceItemGrid(subgrid_id, row_id) {
	...
	$("#"+subgrid_id).html("<table id='"+subgrid_table_id+"' class='scroll'></table><div id='"+pager_id+"' class='scroll'></div>");
	$("#"+subgrid_table_id).jqGrid({
		url: ctx + "/js/module/newPolicy/new-policy-coporation-car-item.json",
		datatype: "json", 
		...
		colModel: [{
			...
			edittype: 'select',
			editoptions: {
				value: {"1": "车辆损失险", "2": "全车盗抢险", "3": "商业三责险", "4": "划痕险", "5": "车辆停驶损失险"}
			}
		}, {
			name: 'companyName'
		}, {
	...
	$('#effectInsuranceTemplate').dialog({
		...
		buttons: [{
			...
			click: function() {
				...
				$('#' + rowId + ' .toVerify').show();
				$(this).dialog('close');
				$(srcEle).parent().html("<a href='#' class='affix' rowId='" + rowId + "'>2个</a>");
				bindAffix();
