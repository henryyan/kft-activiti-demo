...
public class ProcessDiagramCanvas {
	...
	public InputStream generateImage(String imageType) {
		...
		try {
			...
			if (minX >= 0 && minY >= 0) {
				imageToSerialize = processDiagram.getSubimage(minX - 5, minY - 5, canvasWidth - minX + 5, canvasHeight
						- minY + 5);
			}
	...
	public void drawExclusiveGateway(int x, int y, int width, int height) {
		...
		Stroke orginalStroke = g.getStroke();
		g.setStroke(GATEWAY_TYPE_STROKE);
		Line2D.Double line = new Line2D.Double(x + quarterWidth + 3, y + quarterHeight + 3, x + 3 * quarterWidth - 3, y
				+ 3 * quarterHeight - 3);
		g.draw(line);
		line = new Line2D.Double(x + quarterWidth + 3, y + 3 * quarterHeight - 3, x + 3 * quarterWidth - 3, y
				+ quarterHeight + 3);
		g.draw(line);
